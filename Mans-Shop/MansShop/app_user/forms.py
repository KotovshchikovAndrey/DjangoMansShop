...
class ProfileForm(forms.ModelForm):
    ...
    photo = forms.ImageField(
        required=False,
        label="Фото",
        widget=forms.FileInput(attrs={"class": "auth-form-photo"}),
    )
    email = forms.CharField(
        required=True,
        label="Email",
    ...
    balans = forms.CharField(
        required=False,
        label="Баланс",
        widget=forms.NumberInput(attrs={"class": "auth-form-input", "readonly": True}),
    )
    ...
    def clean_username(self):
        ...
        if username != self.instance.username and is_user_exists:
            raise ValidationError(
                "Нельзя установить такое имя, так как оно уже занято!"
            )
    ...
    def clean_email(self):
        ...
        if email != self.instance.email and is_user_exists:
